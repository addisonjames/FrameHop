...
function maybeAutoEnablePageNames() {
  if (showPageName || pageNamesAutoEnabled) return;
...
async function loadPluginData() {
  ...
  if (data) {
    ...
    if (parsedData.settings) {
      ...
      if (historyLength === 4) historyLength = 8;
...
async function updateUI() {
  ...
  const keepViewportHighlight =
    !!previousFav && !!previousFav.isViewport && !currentSelectionId;
  ...
  for (const fav of favorites) {
    ...
    if (node) {
      ...
      const updated = {
        ...
        id: fav.id,
        name: node.name,
        pageId: pageNode ? pageNode.id : fav.pageId,
        pageName: pageNode ? (pageNode as PageNode).name : fav.pageName,
      };
  ...
  for (const item of reversedHistory) {
    ...
    if (node && page) {
      recentHistory.push({
        ...
        pageName:
          figma.editorType === "figma" && showPageName && hasMultiplePages
            ? (page as PageNode).name
            : "",
      });
    }
  }
  ...
  switch (msg.type) {
    ...
    case "autoResize": {
      ...
      const clamped = Math.max(
        ...
        Math.min(computeAutoResizeMaxHeight(), msg.height)
      );
