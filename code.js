...
async function updateUI() {
  ...
  for (const fav of favorites) {
    ...
    // If the node is not found, skip it (filters out deleted favorites)
  }
...
async function hopForwards() {
  console.log("=== hopForwards v7 ENTRY ===");
  ...
  console.log("hopForwards - fwdData exists:", !!fwdData, "currentIndex before re-read:", currentIndex);
  if (fwdData) {
    const parsed = JSON.parse(fwdData);
    console.log("hopForwards - parsed.currentIndex:", parsed.currentIndex, "type:", typeof parsed.currentIndex);
    if (parsed.currentIndex !== undefined) {
      currentIndex = parsed.currentIndex;
      console.log("hopForwards - SET currentIndex to:", currentIndex);
    }
  }
  resolveCurrentIndex();
  console.log(
...
async function cycleHistoryLength() {
  ...
  await updateUI(); // Reflect changes in the UI
