...
async function updateUI() {
    ...
    for (const fav of favorites) {
        ...
        if (fav.isSelection) {
            updatedFavorites.push(fav);
            enrichedFavorites.push(Object.assign(Object.assign({}, fav), { type: "SELECTION" }));
            continue;
        }
...
async function jumpToFrame(frameId) {
    ...
    if (selFav && selFav.selectionIds && selFav.pageId) {
        ...
        if (page && page.type === "PAGE") {
            ...
            if (resolved.length < selFav.selectionIds.length) {
                ...
                figma.notify(`Restored ${resolved.length} of ${selFav.selectionIds.length} items (${missing} missing)`);
            }
            currentFavoriteIndex = favorites.findIndex((f) => f.id === frameId);
            ...
            console.log("Restored selection favorite:", frameId, `(${resolved.length} nodes)`);
        }
        else {
    ...
    if (targetFrame) {
        ...
        if (targetPage) {
            ...
            lastSelectionIds = [targetFrame.id];
            isNavigating = false;
...
async function updateHistory() {
    ...
    if (newItems.length === 0) {
        const anchorIdx = history.findIndex((h) => h.frameId === currentSelection[0].id);
        if (anchorIdx !== -1)
